...
alt.on('update', () => {
    if (police.handcuffed) {
        alt.log(game.isEntityPlayingAnim(alt.Player.local.scriptID, anim.dict, anim.name, 3))
        if (!game.isEntityPlayingAnim(alt.Player.local.scriptID, anim.dict, anim.name, 3)) {
            police.putHandcuff();
        }
        game.setPedPathCanUseLadders(alt.Player.local.scriptID, false)
